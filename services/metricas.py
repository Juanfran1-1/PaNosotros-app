...
def calcular_metricas(df):
    try:
        ...
        # 2. Manejo de Fechas
        # Convertimos la columna fecha a datetime (manejando UTC de Supabase)
        df["fecha_dt"] = pd.to_datetime(df["fecha"], errors='coerce')
        
        # Obtenemos la fecha de hoy en formato texto YYYY-MM-DD
        # Esto evita problemas si el servidor tiene otra zona horaria
        hoy_texto = datetime.now().strftime('%Y-%m-%d')
        
        # Creamos una columna de solo fecha en el dataframe
        ...
        digital = df_mes[df_mes["metodo_pago"].isin(["Digital", "Virtual", "Transferencia"])]["monto"].sum()
